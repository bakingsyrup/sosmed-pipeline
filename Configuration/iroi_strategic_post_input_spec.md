...
1. **Phase 5.1 (Wireframe Drafting Engine)**: ✅ Complete. `draft_agent.mjs` + `wireframe_drafting_prompt.mjs` with Editor-in-Chief workflow — Planner→Researcher→Writer's Proposal (reviewed by human) → Drafter. Supports `format` filter (`thread`/`single_post`/`article`), `post_count`, `custom_styles` with index-based padding, per-platform style cooldown (10-post window per platform), and blueprint compression. Monitors `_NEW_STRATEGIC_INPUT.md` in `x-pipeline/01-Inbox/00-Strategic-Inputs/`. **Only the X (Twitter) pipeline is wired.** Templates at `ig-pipeline/`, `tiktok-pipeline/`, `linkedin-pipeline/`, and `yt-pipeline/` exist but are not watched — future work.
   - **Status flow**: `draft` → `ready` → (proposal phase) → `awaiting_feedback` → `approved` (drafter runs) or `revise` (re-proposal).
   - **Multi-topic support**: Fresh inputs are saved as `{core_topic}.md` files in `00-Strategic-Inputs/`; the blank `_NEW_STRATEGIC_INPUT.md` is regenerated immediately so multiple topics can be in-flight simultaneously. Named files trigger on `approved` or `revise` (not `ready`).
   - **Writer's Proposal structure** (in order): Opening → Style recommendation → Thread composition (segments with role + emphasis) → Quick recap (hook/body/close per post) → Top 10 facts → Detailed post flow. Meta-commentary in English; post content examples in target language (Indonesian).
   - **Style dedup**: Reads last 10 X-platform published posts from `topic_history_log.json`, excludes those styles during auto-selection.
   - **Article support**: Requires `_Article_`-named style files in the Style Bank. Dissected via `_NEW_POST_DISSECTION.md` with `format: article`.
   - **Drafter scope**: Generates 1 draft variation (not 4) — reduced to minimize DeepSeek API calls after approval.
   - **Known limitation (DeepSeek reliability)**: DeepSeek intermittently resets connections (`ECONNRESET`) on large prompts (3-5KB+). Not a code bug — reproduced in standalone scripts with zero prior state. Small prompts never abort. The strategic workflow makes 2-3 large DeepSeek calls per run (style selection if auto, planner, proposal) + 1 drafter call after approval. Retries eventually succeed but are slow. A future merge of Planner+Researcher into a single Gemini call would reduce DeepSeek calls to 1 per phase. Not yet implemented.
