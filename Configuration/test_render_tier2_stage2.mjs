...
const TEMPLATE_DIR = path.join(__dirname, 'templates', 'instagram', 'body_cards', 'tier2_standard');
const OUTPUT_DIR = path.join(__dirname, 'templates', 'instagram', 'previews', 'tier2_gallery');

async function runStage2CalibrationTest() {
  ...
  console.log(`\n======================================================`);
  console.log(`🚀 STAGE 2: TIER 2 CALIBRATED GALLERY RENDERING`);
  console.log(`======================================================\n`);

  // Format 1: Bullet SOP (Calibrated Tier 2)
  {
    const tpl = fs.readFileSync(path.join(TEMPLATE_DIR, 'Format_1_BulletSOP_T2.html'), 'utf8');
    ...
    const outPath = path.join(OUTPUT_DIR, 'Slide_2_Format_1_BulletSOP_T2.png');
    await page.setContent(html, { waitUntil: 'networkidle' });
    await page.screenshot({ path: outPath, type: 'png' });
    console.log(`   ✓ Rendered Stage 2 Format 1: ${outPath}`);
  }

  // Format 2: Key-Value Table (Calibrated Tier 2)
  {
    const tpl = fs.readFileSync(path.join(TEMPLATE_DIR, 'Format_2_KeyValueTable_T2.html'), 'utf8');
    ...
    const outPath = path.join(OUTPUT_DIR, 'Slide_3_Format_2_KeyValueTable_T2.png');
    await page.setContent(html, { waitUntil: 'networkidle' });
    await page.screenshot({ path: outPath, type: 'png' });
    console.log(`   ✓ Rendered Stage 2 Format 2: ${outPath}`);
  }

  // Format 3: Arrow Flow (Calibrated Tier 2)
  {
    const tpl = fs.readFileSync(path.join(TEMPLATE_DIR, 'Format_3_ArrowFlow_T2.html'), 'utf8');
    ...
    const outPath = path.join(OUTPUT_DIR, 'Slide_4_Format_3_ArrowFlow_T2.png');
    await page.setContent(html, { waitUntil: 'networkidle' });
    await page.screenshot({ path: outPath, type: 'png' });
    console.log(`   ✓ Rendered Stage 2 Format 3: ${outPath}`);
  }

  // Format 4: Q&A Split (Calibrated Tier 2)
  {
    const tpl = fs.readFileSync(path.join(TEMPLATE_DIR, 'Format_4_QASplit_T2.html'), 'utf8');
    ...
    const outPath = path.join(OUTPUT_DIR, 'Slide_5_Format_4_QASplit_T2.png');
    await page.setContent(html, { waitUntil: 'networkidle' });
    await page.screenshot({ path: outPath, type: 'png' });
    console.log(`   ✓ Rendered Stage 2 Format 4: ${outPath}`);
  }

  // Format 5: Narrative Block (Calibrated Tier 2)
  {
    const tpl = fs.readFileSync(path.join(TEMPLATE_DIR, 'Format_5_NarrativeBlock_T2.html'), 'utf8');
    ...
    const bodyHtml = `
      ...
      <div class="narrative-body">Sebagian besar trader pemula kehilangan seluruh modalnya bukan karena analisis teknikal yang buruk, melainkan karena keserakahan menggunakan leverage tinggi tanpa menghitung batas risiko kerugian maksimal.</div>
    `;
    let html = tpl
      .replace(/\{\{ACCENT_HEX\}\}/g, '#10B981')
      .replace(/\{\{HEADLINE_TEXT\}\}/g, headline)
    ...
    const outPath = path.join(OUTPUT_DIR, 'Slide_6_Format_5_NarrativeBlock_T2.png');
    await page.setContent(html, { waitUntil: 'networkidle' });
    await page.screenshot({ path: outPath, type: 'png' });
    console.log(`   ✓ Rendered Stage 2 Format 5: ${outPath}`);
  }

  await browser.close();

  console.log(`\n======================================================`);
  console.log(`✅ STAGE 2 COMPLETE. Output files saved to: ${OUTPUT_DIR}`);
  console.log(`======================================================\n`);
}

runStage2CalibrationTest().catch(console.error);
