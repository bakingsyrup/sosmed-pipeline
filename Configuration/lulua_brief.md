...
* **Status**: Steps 1, 2, 3 fully built; dual wireframe (launch post + X Article) live; Grok redirect bypass live; "Show more" expansion fixed (main-column scoped); author handle extraction unified for absolute/relative URLs with reserved routes blacklist; multi-platform enabled; integrated into Ad Astra Web UI. *Open: Beidou cache bypass for truncated tweets with `…` suffix.*
