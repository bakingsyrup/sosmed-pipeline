...
* **Status**: Steps 1, 2, 3 fully built; dual wireframe (launch post + X Article) live; Grok redirect bypass live; "Show more" expansion fixed; multi-platform enabled (X, IG, YT, TikTok); integrated into Ad Astra Web UI with MOC index management. *Active debugging: author handle extraction for posts rendered with absolute URLs.*
* **August 1, 2026**: Upgraded **Smart Article Extraction & Grok Redirect Bypass** (`lulua_url_fetcher.mjs`): Fixed X Article body extraction by switching from `<article>`-scoped text collection to `body.innerText` with date+stats footer boundary detection (2558→8801 chars). Added **Grok share redirect bypass** — detects JS redirects to `/i/grok/share/`, then fetches raw status page HTML via backend `fetch()` to extract the canonical article ID from `sharedContent` schema.org `<meta>` tags. **Fixed 4-strategy article URL detection** (`findArticleUrl`) with proper priority: CSS selectors → meta tags → innerHTML regex (relative + absolute URLs) → data-href attrs; click-required fallback defers to later strategies. **Fixed "Show more" expansion** — replaced `article[data-testid="tweet"]`-scoped selectors with page-wide `a[role="link"], button, span[role="button"]` search. **Fixed author handle extraction** — unified regex `/(?:x\.com\/|\/)([A-Za-z0-9_]{1,15})(?:\/|$)/i` handles both absolute (`https://x.com/user`) and relative (`/user`) X profile URLs. *Open: Beidou cache bypass for truncated tweets with `…` suffix; lookonchain post dissection debug in progress.*
