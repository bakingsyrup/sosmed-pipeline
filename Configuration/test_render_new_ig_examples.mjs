...
async function renderNewExamples() {
  ...
  console.log(`Found ${files.length} new image files in ${INPUT_DIR}:`, files);
  ...
  for (let idx = 0; idx < files.length; idx++) {
    ...
    for (const p of placements) {
      count++;
      const fileName = `Slide_1_new_img${idx+1}_${baseName}_Cover_${p.name}.png`;
      console.log(`[${count}/${files.length * 4}] Rendering ${fileName}...`);
  ...
  await browser.close();
  console.log(`\n[Done] Successfully rendered ${count} cover files to: ${TARGET_GALLERY_DIR}`);
}

renderNewExamples().catch(console.error);
