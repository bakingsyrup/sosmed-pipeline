...
2. **[utils/youtube_clipper.mjs](file:///mnt/data/Obsidian%20Docs/Image%20Prompt%20Db/Sosmed-Pipeline/Configuration/utils/youtube_clipper.mjs)**: Node.js wrapper invoking `scripts/yt_clip_finder.py` to search, subtitle-scan, and clip 30-60s `.mp4` video clips during Research Phase.
...
* **Status**: `x-draft-agent` actively running via PM2 in watch mode; syncs real-time state with Ad Astra UI (`00-Status.md`).
